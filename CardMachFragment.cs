        ...
        private void UndoButtonOnClick()
        {
            try
            {
                ...
                if (AppSettings.EnableAppFree || dataUser?.IsPro != "0")
                {
                    ...
                    Toast.MakeText(Context, "Become Premium", ToastLength.Short)?.Show();
            }
            catch (Exception exception)
            {
                Methods.DisplayReportResultTrack(exception);
            ...
            try
            {
                if (TotalCount >= 2 || CardDateAdapter.UsersDateList.Count == 0)
                {
                    if (ListUtils.LikedList.Count > 0)
                    {
                        ...
                        RequestsAsync.Users.AddLikesAsync(TotalIdLiked, "")
                            .ContinueWith(task =>
                            {
                                if (task.Result.Item1 == 200)
                                {
                                    foreach (var item in ListUtils.LikedList)
                                    {
                                        OneSignalNotification.Instance.PostLikeNotificationToUser(item.Email);
                                    }
                                }
                            })
